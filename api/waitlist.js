...
module.exports = async (req, res) => {
  ...
  if (!key || !listId) {
    ...
    return res.status(503).json({
      error: 'unconfigured',
      missing,
      // Diagnostics: names and value LENGTHS only — never the values themselves.
      // A name present with length 0 means the variable exists in Vercel but
      // its value was saved empty, which is invisible in the dashboard.
      seen: Object.keys(process.env)
        .filter((k) => /MAIL|CHIMP/i.test(k))
        .sort()
